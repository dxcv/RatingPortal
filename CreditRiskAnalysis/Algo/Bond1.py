...
class Bond1(GeneralBond):
    ...
    def __init__(self, bond_code, other_score=None):
        ...
        self.year_len = 3
        self.raw_data_template = 'Bond1_RawDataFields'
        self.indicator2score_criterion = 'Bond1_Indicator2Score'
        self.factor_weight = 'Bond1_FactorWeight'
        self.score2rating_criterion = 'Bond1_Score2Rating'
    ...
    def calc_indicators(self):
        ...
        indicators.ix["(现金-短债)/净资产, 含应付应收"] = (
                                                  ((self.treated_data.ix["MONETARY_CAP"] + self.treated_data.ix["TRADABLE_FIN_ASSETS"]
                                                    ...
                                                    + self.treated_data.ix["NON_CUR_LIAB_DUE_WITHIN_1Y"]
                                                  )) / self.treated_data.ix["TOT_EQUITY"])[start_col:]

        indicators.ix["(现金-短债)/净资产，含有息债务"] = (((self.treated_data.ix["MONETARY_CAP"] + self.treated_data.ix["TRADABLE_FIN_ASSETS"] + self.treated_data.ix["NON_CUR_ASSETS_DUE_WITHIN_1Y"])
                                                - (self.treated_data.ix["ST_BORROW"] + self.treated_data.ix["BORROW_CENTRAL_BANK"] + self.treated_data.ix["NON_CUR_LIAB_DUE_WITHIN_1Y"])
                                            ) / self.treated_data.ix["TOT_EQUITY"])[start_col:]
        ...
        indicators.ix["有息负债变化值"] = np.array(indicators.ix["有息负债率"]) - np.array(
            (self.treated_data.ix["ST_BORROW"] + self.treated_data.ix["BORROW_CENTRAL_BANK"] + self.treated_data.ix["NON_CUR_LIAB_DUE_WITHIN_1Y"]
             + self.treated_data.ix["LT_BORROW"] + self.treated_data.ix["BONDS_PAYABLE"]) / self.treated_data.ix["TOT_ASSETS"])[start_col - 1: col_num - 1]
        ...
        indicators.ix["EBITDA/总债务"] = (indicators.ix["EBITDA"] /
                                       ((self.treated_data.ix["ST_BORROW"] + self.treated_data.ix["BORROW_CENTRAL_BANK"]
                                         + self.treated_data.ix["TRADABLE_FIN_LIAB"] + self.treated_data.ix["NOTES_PAYABLE"]
                                         + self.treated_data.ix["ACCT_PAYABLE"] + self.treated_data.ix["HANDLING_CHARGES_COMM_PAYABLE"]
                                         + self.treated_data.ix["EMPL_BEN_PAYABLE"] + self.treated_data.ix["TAXES_SURCHARGES_PAYABLE"]
                                         ...
                                         + self.treated_data.ix["NON_CUR_LIAB_DUE_WITHIN_1Y"] + self.treated_data.ix["LT_BORROW"]
                                         + self.treated_data.ix["BONDS_PAYABLE"] + self.treated_data.ix["LT_PAYABLE"]
                                         + self.treated_data.ix["SPECIFIC_ITEM_PAYABLE"]) / unit))[start_col:]
        ...
        indicators.ix["未使用授信/总债务"] = self.treated_data.ix["CREDIT_UNUSED"] / \
                                     ((self.treated_data.ix["ST_BORROW"] + self.treated_data.ix["BORROW_CENTRAL_BANK"] + self.treated_data.ix["TRADABLE_FIN_LIAB"]
                                       + self.treated_data.ix["NOTES_PAYABLE"] + self.treated_data.ix["ACCT_PAYABLE"] + self.treated_data.ix["HANDLING_CHARGES_COMM_PAYABLE"]
                                       + self.treated_data.ix["EMPL_BEN_PAYABLE"] + self.treated_data.ix["TAXES_SURCHARGES_PAYABLE"]
                                       + self.treated_data.ix["INT_PAYABLE"] + self.treated_data.ix["OTH_PAYABLE"]
                                       + self.treated_data.ix["NON_CUR_LIAB_DUE_WITHIN_1Y"] + self.treated_data.ix["LT_BORROW"]
                                       + self.treated_data.ix["BONDS_PAYABLE"] + self.treated_data.ix["LT_PAYABLE"]
